document.addEventListener('DOMContentLoaded', () => {
  ...
  const reviewGuidance={
    'Interpretación':'La decisión debe integrar las evidencias relevantes del caso, distinguir condiciones del contexto y evitar conclusiones basadas en un único antecedente.',
    'Aplicación':'El apoyo debe responder a la barrera identificada, conservar el aprendizaje central y permitir comprobar su efecto en una situación comparable.',
    'Decisión pedagógica':'La alternativa más sólida articula antecedentes, propósito, acciones e indicadores de seguimiento, evitando decisiones automáticas o irreversibles.'
  };
  const escapeHTML=value=>String(value).replace(/[&<>'"]/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[char]));
  const renderAnswerReview=(data)=>{
    const review=document.querySelector('#answer-review');
    if(!review)return;
    const incorrect=questions.map((question,index)=>({question,index,selected:Number(data.get(`q${index}`))})).filter(item=>item.selected!==item.question[3]);
    review.innerHTML=incorrect.length?incorrect.map(({question,index,selected})=>{
      const moduleLabel=question[4]?` · ${escapeHTML(question[4])}`:'';
      const guidance=reviewGuidance[question[0]]||'Contrasta todos los antecedentes del caso y selecciona la alternativa que responda con mayor precisión al propósito pedagógico planteado.';
      const escapedGuidance=escapeHTML(guidance);
      return '<article><p class="review-meta">Pregunta '+(index+1)+' · '+escapeHTML(question[0])+moduleLabel+'</p><h4>'+escapeHTML(question[1])+'</h4><p><strong>Tu respuesta:</strong> '+escapeHTML(question[2][selected])+'</p><p class="review-correct"><strong>Respuesta esperada:</strong> '+escapeHTML(question[2][question[3]])+'</p><p class="review-guidance"><strong>Orientación:</strong> '+escapedGuidance+'</p></article>';
    }).join(''):'<p class="answer-review-empty">¡Excelente! No tienes respuestas incorrectas en este intento.</p>';
  };
